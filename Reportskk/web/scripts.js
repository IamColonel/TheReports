...
function viewFeedbackWindow(feedbackid) {
	...
	if (data.category === 'player_report') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-user-alt"></i> REPORT JOUEUR</span>`)
...
function reloadFeedbackTable() {
	...
	$.each(FeedbackTable, function(id, feedback) {
		...
		if (feedback.category === 'player_report') {
			category = "<span><i class='fas fa-user-alt'></i> REPORT JOUEUR</span>"
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					...
					<td><button type="button" class="btn btn-view"><i class="fas fa-eye"></i> VOIR</button></td>
				</tr>`
			)
		}
	});
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("BEAU TRAVAIL ! VOUS POUVEZ MAINTENANT VOUS DÉTENDRE ET PRENDRE UNE GORGÉE.");
		...
		$(".feedback_subtitle").html("HÉ ! ON DIRAIT QUE VOUS AVEZ DU TRAVAIL À FAIRE...");
